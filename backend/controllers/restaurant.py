...
def get_all_restaurants():
    try:
        ...
        # # productCollection.append(get_all_prods())
        # print("------------------------")
        # print(productCollection)
        # print("----------------------------")
        return jsonify({"status":"Success","data": [{"_id":"1", "name":"Chipotle", "products":get_all_prods()}]}), 200
...
def get_all_prods():
    try:
        products = list(mongo.db.Menu.find({}))
        for prod in products:
            prod['_id'] = str(prod['_id'])

            prod['restaurantId'] = str(prod.get('restaurantId', 1))
