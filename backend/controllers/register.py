...
def register_user():
    try:
        ...
        isEmployee = registration_data["isEmployee"]
        registration_data = registration_data['data']

        # errors = register_schema.validate(registration_data["data"])
        # if errors:
        ...
        ssn = registration_data.get('ssn')
        street = registration_data.get('street')
        city = registration_data.get('city')
        state = registration_data.get('state')
        zipCode = registration_data.get('zipCode')
        ...
        user_data = {
            ...
            'dob':dob,
            'gender':gender,
            'role': "emp" if isEmployee else "customer"
        }  
        if isEmployee:
            user_data['ssn'] = ssn
